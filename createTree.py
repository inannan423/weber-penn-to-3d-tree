...
class stemSpline:
    def __init__(self, spline, curvature, curvatureV1,curvatureV2, attractUp, segments, maxSegs,
                 segLength, childStems, stemRadStart, stemRadEnd, splineNum, ofst, pquat):
        self.spline = spline
        self.p = spline.bezier_points[-1]
        self.curv = curvature
        self.curvV1 = curvatureV1
        self.curvV2 = curvatureV2
...
def toDeg(list):
    return [degrees(a) for a in list]
...
def growSpline(n, stem, numSplit, splitAng, splitAngV1, splitAngV2, splineList,
               hType, splineToBone, closeTip, kp, splitHeight, outAtt, stemsegL,
               lenVar, taperCrown, boneStep, rotate, rotateV, choiceNegPos1, choiceNegPos2, rLastAng):
    ...
    curveangle = sCurv + (stem.curvV1 * kp * stem.curvSignx)
    curveVar = stem.curvV2 * kp * stem.curvSigny
    stem.curvSignx *= -1
    stem.curvSigny *= -1
    ...
    if numSplit > 0:
        ...
        for i in range(numSplit):
            ...
            # Spread the stem out in a rand fashion
            ...
            nstem = stemSpline(
                        newSpline, stem.curv, stem.curvV1, stem.curvV2, stem.vertAtt, stem.seg + 1,
...
def genLeafMesh(leafScale, leafScaleX, leafScaleT, leafScaleV, loc, quat,
    ...
    if rotate < 0.0:
        oldRot = -copysign(rotate + rotateV, oldRot)
    else:
    ...
    leafScale = leafScale * leafScaleV
...
def create_armature(armAnim, leafP, cu, frameRate, leafMesh, leafObj, leafVertSize, leaves,
                    levelCount, splineToBone, treeOb, wind, gust, gustF, af1, af2, af3, af4,
    ...
    for i, parBone in enumerate(splineToBone):
        if (i < levelCount[armLevels]) or (armLevels == -1) or (not makeMesh):
            ...
            if armAnim:
                ...
                # Set the rand phase difference of the animation
...
def kickstart_trunk(addstem, levels, leaves, branches, cu, curve, curveRes,
                    curveV1, curveV2, attractUp, length, lengthV, ratio, ratioPower,
    ...
    startRad = scaleVal * ratio * scale0 * scaleV0  # * (scale0 + uniform(-scaleV0, scaleV0))
    ...
    addstem(
        stemSpline(
                newSpline, curveVal, curveV1[0], curveV2[0], attractUp[0],
...
def fabricate_stems(addsplinetobone, addstem, baseSize, branches, childP, cu, curve, curveBack,
                    curveRes, curveV1, curveV2, attractUp, downAngle, downAngleV, leafDist, leaves, length,
    ...
    for i, p in enumerate(childP):
        ...
        if useOldDownAngle:
            ...
            # Otherwise just find a rand value
            else:
                downV = downAngleV[n]
        else:
            if downAngleV[n] < 0.0:
        ...
        if rotate[n] < 0.0:
            oldRotate = -copysign(rotate[n], oldRotate)
        # Otherwise just generate a rand number in the specified range
        else:
            oldRotate += rotate[n]
        # bRotate = oldRotate + uniform(-rotateV[n], rotateV[n])
        bRotate = oldRotate + rotateV[n]
        ...
        # print("p.lengthPar: ", p.lengthPar)
        # if(p.lengthPar <= 0):
        #     p.lengthPar = lastLengthPar
        # lastLengthPar = p.lengthPar
        ...
        addstem(
            stemSpline(
                newSpline, curveVal, curveVar1, curveVar2, attractUp[n],
...
def perform_pruning(baseSize, baseSplits, childP, cu, currentMax, currentMin, currentScale, curve,
                    ...
                    originalCo, originalCurv, originalCurvV1, originalCurvV2, originalHandleL, originalHandleR, originalLength,
                    originalSeg, prune, prunePowerHigh, prunePowerLow, pruneRatio, pruneWidth, pruneBase,
                    pruneWidthPeak, randState, ratio, scaleVal, segSplits, splineToBone, splitAngle, splitAngleV1, splitAngleV2,
                    st, startPrune, branchDist, length, splitByLen, closeTip, nrings, splitBias, splitHeight,
                    attractOut, rMode, lengthV, taperCrown, boneStep, rotate, rotateV, rSplits2, choiceNegPos1, choiceNegPos2,
                    rLastAng):
    while startPrune and ((currentMax - currentMin) > 0.005):
        ...
        if deleteSpline:
            ...
            st.curvV1 = originalCurvV1
            st.curvV2 = originalCurvV2
            st.seg = originalSeg
            st.p = newPoint
        ...
        # splineList[0].segL = stemsegL * lengthV[n]  # variation for first stem
                ...
                # always split at splitHeight
                    ...
                    if (n >= 1) and splitByLen:
                        ...
                        for l in length[:n + 1]:
                            lf *= l
                        L = L / lf
                        numSplit = splits2(splitVal * L, rSplits2)
                    else:
                        numSplit = splits2(splitVal, rSplits2)

                if (k == int(curveRes[n] / 2 + 0.5)) and (curveBack[n] != 0):
                    spl.curv += 2 * (curveBack[n] / curveRes[n])  # was -4 *

                growSpline(
                        n, spl, numSplit, splitAngle[n], splitAngleV1[n], splitAngleV2[n], splineList,
                        handles, splineToBone, closeTip, kp, splitHeight, attractOut[n],
                        stemsegL, lengthV[n], taperCrown, boneStep, rotate, rotateV, choiceNegPos1, choiceNegPos2,
                        rLastAng)
        ...
        if (((currentMax - currentMin) < 0.005) or not prune) or forceSprout:
            ...
            # arr = []
            # for s in splineList:
            #     arr.append(s.offsetLen + (len(s.spline.bezier_points) - 1) * s.segL)
            #     # print("offsetLen: ", s.offsetLen)
            #     # print(len(s.spline.bezier_points))
            #     # print(s.segL)
            # print(arr)
            for s in splineList:
...
def addTree(props):
    ...
    curveV1 = toRad(props.curveV1)
    curveV2 = toRad(props.curveV2)
    ...
    splitAngleV1 = toRad(props.splitAngleV1)
    splitAngleV2 = toRad(props.splitAngleV2)
    scale = props.scale
    scaleV = props.scaleV
    ...
    rSplits2 = props.rSplits2
    choiceNegPos1 = props.choiceNegPos1
    choiceNegPos2 = props.choiceNegPos2
    rLastAng = props.rLastAng
    ...
    # print("Tree generation started 2")
    ...
    for n in range(levels):
        ...
        if n == 0:
            kickstart_trunk(addstem, levels, leaves, branches, cu, curve, curveRes,
                            curveV1, curveV2, attractUp, length, lengthV, ratio, ratioPower, resU,
        ...
        # If this isn't the trunk then we may have multiple stem to initialise
            ...
            fabricate_stems(addsplinetobone, addstem, baseSize, branches, childP, cu, curve, curveBack,
                            curveRes, curveV1, curveV2, attractUp, downAngle, downAngleV, leafDist, leaves, length, lengthV,
        ...
        for st in stemList:
            ...
            # When using pruning, we need to ensure that the rand effects
            ...
            originalCurvV1 = st.curvV1
            originalCurvV2 = st.curvV2
            originalSeg = st.seg
    ...
    # print("Tree generation started 3")
    ...
    if useArm:
        ...
        create_armature(
                    ...
                    af2, af3, af4, leafAnim, loopFrames, previewArm, armLevels, makeMesh, boneStep
                    )
    ...
    # 导出为 obj 文件
    # bpy.ops.export_scene.obj(filepath=props.output_path + ".obj")

    # 同时导出为 glb 文件
    # bpy.ops.export_scene.gltf(filepath=props.output_path + ".glb")

    # 存储一份 blend 文件
    bpy.ops.wm.save_as_mainfile(filepath=props.output_path + ".blend")

    # 导出 fbx 文件
    bpy.ops.export_scene.fbx(filepath=props.output_path + ".fbx")

    print("export gltf time", time.time() - t1)
